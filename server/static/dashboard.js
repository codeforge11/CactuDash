...
socket.onmessage = function (event) {
    ...
    if (Array.isArray(data.containers)) {
        ...
        data.containers.forEach(container => {
            ...
            removeButton.id = 'restartButton';
            removeButton.title = 'Remove';
            removeButton.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" color="black" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-trash2-icon lucide-trash-2">
                <path d="M3 6h18"/>
            ...
            if (confirm('Are you sure you want to remove: '+container.Name+'?')) {
                fetch('/remove/' + container.Id, { method: 'POST' })
                .then(() => location.reload());
            }
            }
